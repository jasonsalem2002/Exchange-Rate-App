...
for i in range(len(df) - n_input):
    x, y = TimeseriesGenerator(scaled_data, scaled_data, length=n_input, batch_size=1)[i]
    all_predictions.append(model.predict(x)[0][0])

all_predictions_inverse = scaler.inverse_transform(np.array(all_predictions).reshape(-1, 1))

prediction_dates = df['added_date'][n_input:]
predicted_df = pd.DataFrame({
    'added_date': prediction_dates,
    'lbp_amount': all_predictions_inverse.flatten()
})
predicted_df.to_csv('predicted_exchange_rates.csv', index=False)

cutoff_date = pd.to_datetime('2024-05-26')
plt.figure(figsize=(14, 5))
plt.plot(df['added_date'][n_input:], df['lbp_amount'][n_input:], label='True Values', marker='o')
plt.plot(prediction_dates[prediction_dates <= cutoff_date], predicted_df['lbp_amount'][prediction_dates <= cutoff_date], label='Model Predictions (Historic)', marker='x', color='orange')
plt.plot(prediction_dates[prediction_dates > cutoff_date], predicted_df['lbp_amount'][prediction_dates > cutoff_date], label='Model Predictions (Future)', marker='x', color='red')
