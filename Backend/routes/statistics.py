...
def get_average_exchange_rate():
    ...
    for transaction in transactions:
        ...
        if granularity == 'daily':
            ...
            return jsonify({"error": "Invalid granularity. Please choose either 'daily', 'monthly', or 'yearly'."}), 400

        lbp_to_usd_totals[period] += lbp_amount / usd_amount
        usd_to_lbp_totals[period] += usd_amount / lbp_amount
        counts[period] += 1

    average_lbp_to_usd = {str(period): lbp_to_usd_totals[period] / counts[period] for period in lbp_to_usd_totals}
    average_usd_to_lbp = {str(period): usd_to_lbp_totals[period] / counts[period] for period in usd_to_lbp_totals}
