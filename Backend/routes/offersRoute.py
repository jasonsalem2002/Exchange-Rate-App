    ...
    try:
        ...
        if unexpected_fields:
            return (
                jsonify(
                    {
                        "error": f'Unexpected fields in request body: {", ".join(unexpected_fields)}'
                    }
                ),
                400,
            )
        ...
        if not all(
            isinstance(val, (int, float)) for val in [amount_requested, amount_to_trade]
        ) or not isinstance(usd_to_lbp, bool):
            return (
                jsonify({"error": "Invalid data types provided for offer amounts."}),
                400,
            )
...
def get_accepted_offers():
    try:
        ...
        complete_offers = Offer.query.filter_by(
            user_id=user_id, mark_as="complete"
        ).all()
        serialized_offers = []
