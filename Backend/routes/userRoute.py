...
def create_user():
    try:
        ...
        if not re.match(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&_])[A-Za-z\d@$!%*?&_]{8,64}$", password):
            return jsonify({'error': 'Password must be between 8 and 64 characters and contain at least one uppercase letter, one lowercase letter, one digit, and one special character.'}), 400
